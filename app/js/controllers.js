...
unitDb.controllers = {
    homeCtrl: [
        ...
        '$location',
        'data',
        function ($scope, $window, $location, data) {
            $scope.factions = [];
            $scope.kinds = [];
            $scope.tech = [];
            ...
            $scope.compare = function (item) {
                ...
                if (idx === -1) $scope.contenders.push(item.id);
                else $scope.contenders.splice(idx, 1);
            };
            $scope.clear = function () {
                for (var c in $scope.index) if ($scope.index[c].selected) $scope.index[c].selected = false;
            ...
            $scope.strain = function (e) {
                return (
                    ($scope.factions.length === 0 || isInArray($scope.factions, e.faction)) &&
            ...
            $scope.unitClick = function (unit, event) {
                ...
                if (event.ctrlKey) {
                    //The control key is pressed: we open a new page
                    //with only the unit
                    ...
                    if (
                        lastClickUnit === unit && //it a double click: we go to
                        newTime - lastClickTime < maxDoubleClickDelay
                    ) {
                        //compare view
                        if (!unit.selected) $scope.compare(unit);
    ...
    gdiCtrl: [
        ...
        function ($scope, $window, $location, data) {
            ...
            $scope.setBpSelected = function (item, selected) {
                ...
                var idx = $scope.contenders.indexOf(item.id);
                if (idx === -1 && selected) $scope.contenders.push(item.id);
            ...
            $scope.toggleBpSelectedByGdiClass = function (gdiClass) {
                ...
                for (var c in classItems) $scope.setBpSelected(classItems[c], newStateIsSelected);
            };
            $scope.clear = function () {
                for (var c in $scope.index) if ($scope.index[c].selected) $scope.index[c].selected = false;
    ...
    compareCtrl: [
        ...
        '$timeout',
        'data',
        function ($scope, $window, $routeParams, $location, $timeout, data) {
            ...
            $timeout(() => {
                $scope.brickInstance =
                    $scope.brickInstance || $window.helpers.initBricks('div.compare-container section');
