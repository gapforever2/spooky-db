...
// decorator class to make the unit object a bit more usable
unitDb.UnitDecorator = function (blueprint) {
    ...
    const factionIdLookup = {
        UEF: 0,
        Cybran: 1,
        Aeon: 2,
        Seraphim: 3,
    };
    const gdiClassificationLookup = {
    ...
    const gdiBaseClassificationLookupAndOrder = {
        'Construction - Buildpower': [
            ...
            'ACU Engineering Drone',
            'T2 Engineering Drone',
        ...
        Land: [
            ...
            'T2 Mobile Bomb',
            'T2 Crab Egg (Flak)',
            'T3 Main Assault Bot/Tank',
            'T3 Assault Bot',
            'T3 Sniper Bot',
            'T3 Mobile Heavy Artillery',
            'T3 Mobile Missile Platform',
            'T3 Mobile Anti-Air',
            'T3 Mobile Shield Generator',
            'T3 Shield Disruptor',
            'T3 Crab Egg (Engineer)',
            'T3 Crab Egg (Brick)',
            'T3 Crab Egg (Artillery)',
        ],
        Air: [
            'T1 Interceptor',
            'T1 Attack Bomber',
        ...
        Naval: [
            ...
            'T2 Torpedo Boat',
            'T2 Shield Boat',
            'T2 Counter-Intelligence Boat',
            'T3 Submarine Hunter',
            'T3 Battleship',
            'T3 Strategic Missile Submarine',
            'T3 Tactical Submarine',
            'T3 Aircraft Carrier',
            'T3 Battlecruiser',
            'T3 Missile Ship',
        ],
        ...
        'Structures - Weapons': [
            ...
            'T3 Torpedo Ambushing System',
            'T3 Heavy Artillery Installation',
            'T3 Rocket Artillery Installation',
        ...
        'Structures - Support': [
            'T1 Wall Section',
            'T1 Air Staging Facility',
            'T2 Shield Generator',
            'T2 Shield Generator: ED2',
            'T2 Shield Generator: ED3',
            'T2 Stealth Shield Generator',
            'T3 Shield Generator',
            'T2 Shield Generator: ED4',
            'T2 Shield Generator: ED5',
            'T3 Heavy Shield Generator',
            'T3 Heavy Stealth Shield Generator',
        ],
        'Structures - Intelligence': [
            ...
            'T3 Sonar Platform',
            'T3 Perimeter Monitoring System',
            'T3 Quantum Optics Facility',
        ],
        'Structures - Economy': [
            'T1 Mass Extractor',
            'T1 Power Generator',
    ...
    const getTech = (bp) => techLookup[bp.Categories.find((category) => techLookup.hasOwnProperty(category))] || '';
    const getFullName = (u) => (u.name ? u.name + ': ' : '') + (u.tech === 'EXP' ? '' : u.tech + ' ') + u.description;
    const weaponStats = (weapon) => {
        let shots = Number(weapon.ManualFire || false); // number of projectiles
        const rate = weapon.RateOfFire;
        let cycle = +(1 / rate).toFixed(1); // how long it takes between shots
        const ppf = weapon.ProjectilesPerOnFire || 1;
        const salvoSize = +(weapon.MuzzleSalvoSize ?? 1).toFixed(1);
        const salvoDelay = +((weapon.MuzzleSalvoDelay ?? 1) === 0 ? 0 : (weapon.MuzzleSalvoDelay ?? 1) - 0.1).toFixed(
            1,
        );
        const dotPulse = weapon.DoTPulses ?? 1;
        const rackCount = weapon.RackBones ? weapon.RackBones.length : 1;
        const damage = weapon.Damage * ppf * dotPulse;

        // this thing is needed to properly calculate split projectiles.
        // Unfortunately the numbers hardcoded here are not available in the blueprint,
        // but specified in the .lua files for corresponding projectiles.
        const projectileMultiplierLookup = {
        ...
        if ((weapon.ProjectileId || weapon.ProjectileLifetimeUsesMultiplier) && !weapon.ForceSingleFire) {
            if (projectileMultiplierLookup[weapon.ProjectileId]) {
                shots = projectileMultiplierLookup[weapon.ProjectileId];
            } else {
                shots = salvoDelay ? salvoSize : weapon.RackBones[0].MuzzleBones.length;
            ...
            if (weapon.RateOfFire !== 1) {
                ...
                const reloadCharge =
                    weapon.RackSalvoChargeTime !== 0
                        ? Math.floor(weapon.RackSalvoReloadTime / weapon.RackSalvoChargeTime)
                        : 0;
                const salvoReload =
                    weapon.RackSalvoChargeTime > weapon.RackSalvoReloadTime
                        ? weapon.RackSalvoChargeTime
                        : reloadCharge + weapon.RackSalvoReloadTime;
    ...
    const fireCycle = (weapon) => {
        var stats = weaponStats(weapon);
        switch (weapon.BeamLifetime) {
            case 1:
                return (
                    stats.shots + ' beam(s) / ' + stats.cycle + 's, ' + 9 * weapon.Damage * stats.shots + ' dmg total'
                );
            case 0:
        ...
        if (stats.salvoDelay !== 0) {
            ...
            var reload = +(stats.cycle - (projectiles - 1) * stats.salvoDelay || 0).toFixed(1);
            return (
                projectiles +
                ' times 1 projectile / ' +
                stats.salvoDelay +
        ...
        return (
            ...
            (stats.shots > 1 ? 's' : '') +
            ' / ' +
            (stats.cycle === 1 ? '' : Math.round(stats.cycle * 10) / 10) +
            ' sec<br/>' +
            weapon.Damage * stats.shots +
            ' total dmg'
        );
    };
    const beamCycle = (weapon) => {
        if (weapon.BeamCollisionDelay > 0.1) {
            var shots = Math.round(weapon.BeamLifetime / (0.1 + weapon.BeamCollisionDelay));
            var shotText = '';
            if (shots > 1) shotText = '/ ' + weapon.BeamCollisionDelay + 0.1 + ' sec ';
            return shots + ' times ' + weapon.Damage + ' dmg ' + shotText + weapon.Damage * shots + ' dmg total';
            ...
            if (weapon.DoTPulses) {
                return (
                    ...
                    weapon.DoTTime / 10 +
                    ' sec = ' +
                    weapon.damage * weapon.DoTPulses +
                    ' total ' +
                    ((weapon.DoTTime / 10) * weapon.DoTPulses - 0.1) +
                    ' sec total'
                );
            } else {
    ...
    const gdiClassificationLookupFunc = (bp) => {
    ...
    const gdiBaseClassificationLookupAndOrderFunc = (bp) => {
        ...
        for (var baseClass in gdiBaseClassificationLookupAndOrder) {
            ...
            if (indexInContainedClasses > -1) return [baseClass, outerOrder, indexInContainedClasses];
        }
        return ['Unknown', 0, 0];
    };
    const gdiBaseClassificationLookupFunc = (bp) => {
        var ret = gdiBaseClassificationLookupAndOrderFunc(bp);
        return ret[0];
    };
    const gdiOrderFunc = (bp) => {
        var ret = gdiBaseClassificationLookupAndOrderFunc(bp);
    ...
    self.fullName = getFullName(self);

    // some things just don't play along...
    const isSpecialDpsCalculationNeeded = (id) =>
        [
            'UEL0103', // lobo
            'XSL0103', // zthuee
            'DAA0206', // mercy
    ...
    blueprint.Weapon = blueprint.Weapon?.map((weapon) => {
        weapon.dps = getDps(weapon, isSpecialDpsCalculationNeeded(self.id));
